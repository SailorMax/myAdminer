...
if (adminer()->homepage()) {
	if ($_GET["ns"] !== "") {
		...
		if (!$tables_list) {
			...
			$columns["Row_format"] = array(lang('Row Format') . doc_link(array('sql' => 'show-table-status.html')), "create", lang('Alter table'));
			$columns["Avg_row_length"] = array(lang('Avg Row Length') . doc_link(array('sql' => 'show-table-status.html')), "create", lang('Alter table'));
			$columns["Create_options"] = array(lang('Create Options') . doc_link(array('sql' => 'show-table-status.html')), "create", lang('Alter table'));
			if (support("comment")) {
