...
if (!defined('Adminer\DRIVER')) {
	...
	* @return Result|bool
	*/
	function alter_table(string $table, string $name, array $fields, array $foreign, ?string $comment, string $engine, string $collation, string $auto_increment, ?array $partitioning, ?string $row_format, ?string $options) {
		$alter = array();
	...
	*/
	function copy_tables(array $tables, array $views, string $target): bool {
		if ((count($tables) + count($views)) > 1)
			$target_table = "";		queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");
		foreach ($tables as $table) {
