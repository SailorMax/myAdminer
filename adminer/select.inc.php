			...
			foreach (adminer()->rowDescriptions($rows, $foreign_keys) as $n => $row) {
				...
				foreach ($row as $key => $val) {
					if (isset($names[$key])) {
						...
						$editable = !is_array($row[$key]) && is_utf8($val) && $rows[$n][$key] == $row[$key] && !$functions[$key] && !$field["generated"] && $update;
						...
						echo "<td id='$id'" . ($is_number && ($val === null || is_numeric(strip_tags($val)) || $type == "money") ? " class='number'" : "");
...
<?php
				...
				if (Driver::$jush == "sql") //! use insertUpdate() in all drivers
