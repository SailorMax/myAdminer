Adminer Next:
Support table row format display and edit
Fix file-upload plugin to do not clear previous value if it was not touched on edit-page
Fix edit multiple checked records
Recognize emails with name (sample: name <email@foo.com>)
Fix paginator page 1 (first) skin
Fix table pages top links (all pages, which related to selected table, has similar links)
Optimize referencable_primary() for MySQL
Remove duplicates of collation-lists on `Alter table` page. Require JavaScript on client side! But support "nojs=1" argument to use old method.
Add style chooser to select data export
Support export table rows without AI keys
Input binary UUID for MySQL
Output binary UUID in hex
Fix disabled <select> UI in Chrome
Add detect `SQL command` page when URL containt '&table=' (allow to use UX plugin sql_command_table_fields)
Support receive partial data for alter table (workaround for max_input_vars limit errors) (allow to use UX plugin table_edit_by_fields)
Add possibility to remove conditions in search by fields form
UX plugins for simpler migrate phpMyAdmin users:
 ...
 - executed_query_output_modifier:	<code> text wrap + possibility to show full query + possibility to always show executed query
